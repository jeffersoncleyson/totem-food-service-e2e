...
// ######### BEGIN @ID-0001
Given(
  "The Administrator wants to create category with name: {string}",
...
Then(
  "The response with a new category name: {string}",
...
// ######### BEGIN @ID-0002
When(
  "The Administrator try create a new category with same name",
  async function () {
    this.responseSameCategory = await CategoryService.createCategory(
      this.body,
      this.headers
    );
  }
...
Then(
  ...
  async function (httpStatusCode: number) {
    ...
    StepDefinitionUtil.expectTobeEqual(
      this.responseSameCategory?.status,
      httpStatusCode
    );
  }
);
...
// ######### BEGIN @ID-0003
Given(
  "The Administrator wants to update category with name: {string}",
  async function (name: string) {
...
When("The Administrator update a category", async function () {
  this.response = await CategoryService.updateCategory(
    this.response?.body?.id,
    this.body,
    this.headers
  );
...
Then(
  "The response updating a category name: {string}",
  async function (param: string) {
    StepDefinitionUtil.expectTobeNotNull(this.response);
    const { body } = this.response;
    const { id, name, modifiedAt, createAt } = body;
...
// ######### BEGIN @ID-0004
Given(
  "The Administrator wants to delete category with id: {string}",
